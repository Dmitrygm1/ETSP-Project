...
def main() -> None:
    parser = argparse.ArgumentParser(description="Minimal SER runner (MERaLiON-SER-v1).")
    ...
    timeline, label, scores, vad = predict_emotion_windows(audio, window_s=args.window, hop_s=args.hop)
    ...
    print("mean_vad:", {k: round(v, 4) for k, v in vad.items()})
    if timeline:
        t0, last_label, last_conf, (v, a, d), _ = timeline[-1]
        print(f"current: {last_label} ({last_conf:.2f}) at {t0:.1f}s | V/A/D: {v:.2f}/{a:.2f}/{d:.2f}")
